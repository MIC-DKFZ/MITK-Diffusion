  ...
  OutputType *grad ) const
{
  ...
  for( unsigned int d = 0; d < ImageDimension; d++ )
    ...
    int begin = std::fmax( 0, static_cast<int>( std::floor( cindex[d] -
      this->m_BoundingBoxStart[d] - this->m_CutoffDistance[d] ) ) );
    int end = std::fmin( boundingBoxSize, static_cast<int>( std::ceil(
  ...
  bool evaluateGradient ) const
{
  ...
  int boundingBoxSize = static_cast<int>(
    ...
    0.5 );
  int begin = std::fmax( 0, static_cast<int>( std::floor( cindex -
    this->m_BoundingBoxStart[dimension] -
    this->m_CutoffDistance[dimension] ) ) );
  int end = std::fmin( boundingBoxSize, static_cast<int>( std::ceil( cindex -
    this->m_BoundingBoxStart[dimension] +
    this->m_CutoffDistance[dimension] ) ) );
  ...
  if( evaluateGradient )
    {
    g_last = vnl_math::two_over_sqrtpi * std::exp( -std::sqrt( t ) );
    }
  ...
  for( int i = begin; i < end; i++ )
    ...
    if( evaluateGradient )
      {
      RealType g_now = vnl_math::two_over_sqrtpi * std::exp( -std::sqrt( t ) );
